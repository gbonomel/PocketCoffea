...
def lepton_selection_loose(events, lepton_flavour, params):

    leptons = events[lepton_flavour]
    cuts = params.object_preselection[lepton_flavour+"_loose"]
    # Requirements on pT and eta
    passes_eta = abs(leptons.eta) < cuts["eta"]
    passes_pt = leptons.pt > cuts["pt"]
    ...
    if lepton_flavour == "Electron":
        ...
        if "iso" in cuts.keys():
            passes_iso = leptons.pfRelIso03_all < cuts["iso"]
        passes_id_medium = (leptons[cuts['id']] > cuts['wp']['medium'])
        fails_id_tight = (leptons[cuts['id']] < cuts['wp']['tight'])

        good_leptons = passes_eta & passes_pt & passes_SC & passes_iso & passes_id_medium & fails_id_tight

    elif lepton_flavour == "Muon":
        # Requirements on isolation and id
        passes_iso = leptons.pfRelIso04_all < cuts["iso"]
        passes_id_loose = leptons[cuts['id_loose']] == True
        fails_id_tight  = leptons[cuts['id_tight']] == False

        good_leptons = passes_eta & passes_pt & passes_iso & passes_id_loose & fails_id_tight
